...
namespace Microsoft.Datasync.Client.Offline
{
    ...
    public interface IOfflineStore : IDisposable
    {
        ...
        /// <summary>
        /// Returns a single deserialized item by the ID of the item.
        /// </summary>
        /// <param name="tableName">The table name holding the item.</param>
        /// <param name="id">The ID of the item.</param>
        /// <param name="cancellationToken">A <see cref="CancellationToken"/> to observe.</param>
        /// <returns>A task that returns the item when complete.</returns>
        Task<T> GetItemAsync<T>(string tableName, string id, CancellationToken cancellationToken = default) where T : IQuickDeseriable, new();
        ...
        /// <summary>
        /// Returns deseriablized items from the table.
        /// </summary>
        /// <param name="query">A query describing the items to be returned.</param>
        /// <param name="cancellationToken">A <see cref="CancellationToken"/> to observe.</param>
        /// <returns>A task that returns a page of items when complete.</returns>
        Task<Page<T>> GetPageAsync<T>(QueryDescription query, CancellationToken cancellationToken = default) where T : IQuickDeseriable, new();
        ...
        Task UpsertAsync<T>(string tableName, IEnumerable<T> items, bool ignoreMissingColumns, CancellationToken cancellationToken = default) where T : IBaseModel, new();
    }
}